...
import org.levimc.launcher.core.stats.PlaytimeStore;
 ...
 public class MainActivity extends BaseActivity {
    ...
    private android.widget.TextView homeAccountStatusText;
    private android.widget.TextView playtimeText;
    ...
    private void initAccountHeader() {
        ...
        homeAccountStatusText = findViewById(R.id.home_account_status_text);
        ...
        playtimeText = findViewById(R.id.playtime_text);
        updatePlaytimeUI();
    }

    private void updatePlaytimeUI() {
        if (playtimeText == null) return;
        long totalMs = PlaytimeStore.getTotalMillis(this);
        playtimeText.setText(PlaytimeStore.formatDuration(totalMs));
    }

    private void showProgressLoader(boolean indeterminate) {
        if (binding == null) return;
        android.widget.ProgressBar pb = binding.progressLoader;
        ...
        if (pb.getVisibility() != View.VISIBLE) {
            ...
            pb.animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(180).start();
        }
    }
    ...
    private void hideProgressLoader() {
        ...
        android.widget.ProgressBar pb = binding.progressLoader;
        if (pb.getVisibility() == View.VISIBLE) {
            pb.animate().alpha(0f).scaleX(0.7f).scaleY(0.7f).setDuration(150)
                    .withEndAction(() -> {
                        if (binding != null) binding.progressLoader.setVisibility(View.GONE);
                    }).start();
        }
    }

    private void setProgressLoaderValueAnimated(int progress) {
        if (binding == null) return;
        android.widget.ProgressBar pb = binding.progressLoader;
        android.animation.ObjectAnimator animator = android.animation.ObjectAnimator.ofInt(
                pb, "progress", pb.getProgress(), progress);
    ...
    private void refreshAccountHeaderUI() {
        ...
        if (active == null) {
            ...
            if (homeAccountStatusText != null) homeAccountStatusText.setText("Not signed in");
        } else {
            ...
            if (homeAccountStatusText != null) {
                String displayName = AccountTextUtils.displayNameOrNotSigned(this, active);
                homeAccountStatusText.setText("Signed in as " + displayName);
            }
        }
    }
    ...
    private void showAccountSwitchPopup(View anchor) {
        ...
        ChromaTextEffect.apply(headerName, active != null && active.chromaNameEnabled);
    ...
    protected void onResume() {
        ...
        if (launchSessionStartMs > 0L) {
            ...
            if (sessionMs > 0L) {
                PlaytimeStore.addSessionMillis(this, sessionMs);
            }
        }
        updatePlaytimeUI();
        if (StorageMigrationService.isMigrationRunning(this)) {
    ...
    private void handleIncomingFiles() {
        ...
        fileHandler.processIncomingFilesWithConfirmation(intent, new FileHandler.FileOperationCallback() {
            ...
            public void onProgressUpdate(int progress) {
                ...
                setProgressLoaderValueAnimated(progress);
            }
        }, false);
    }
